...
gg <- ggplot(crit., aes(x=m, y=value, group=variable))+
  ...
  ggtitle("BIC and ICL methods for Velib data (7 dimentions)")
gg
...
ggplot(dfpl, aes(x=lat, y=long)) +
  ...
  ggtitle("Groups predicted by BIC and ICL methods for the Velib data (7 dimensions)")
...
ggplot(dfpl, aes(x=lat, y=long)) +
  geom_point(aes(color=factor(group))) +
    ggtitle("Groups predicted by The LRT method for the Velib data (7 dimensions)")


# plot the mixing proportions:
mixprop <- data.frame(group=c("1", "2", "3", "4"),
                      proportion=pi.est[[m.est]])

ggplot(data=mixprop, aes(x=group, y=proportion, fill=group)) +
  geom_bar(stat="identity") + 
    ggtitle("Mixing proportions computed via the LRT method for the Velib data (7 dimensions)")
